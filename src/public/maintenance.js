document.addEventListener('DOMContentLoaded', () => {
  ...
  // Inventory Maintenance Elements
  ...
  function showMaintenanceSection(sectionId) {
    ...
    else if (selectedOption && selectedOption.display === 'Inventory Maintenance') {
      showInventoryLoading();
      otherMaintenance.classList.remove('hidden');
      maintenanceTitle.textContent = 'Inventory Maintenance';
  ...
  function showInventoryLoading() {
    maintenanceContent.innerHTML = `
      <div class="text-center py-4">
        <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-yellow-500 mx-auto"></div>
        <p class="mt-2 text-gray-600">Loading inventory transactions...</p>
      </div>`;
  }
 ...
 // ======================
  ...
  async function loadInventoryTransactions() {
    try {
      ...
      showInventoryTransactionsError(err.message);
    }
  }
  ...
  function renderInventoryTransactions() {
    maintenanceContent.innerHTML = `
      <div class="flex justify-between items-center mb-4">
        ...
        <button id="addInventoryBtn" class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">
          ...
          <tbody id="inventoryTable" class="bg-white divide-y divide-gray-200">
            ${inventoryTransactions.length > 0 ? 
              inventoryTransactions.map(tx => `
                <tr>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${tx.transaction_id}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${tx.ingredient_name}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${tx.transaction_type}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${safeToFixed(tx.quantity)}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${tx.unit}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${tx.notes || 'N/A'}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <button class="edit-inventory text-yellow-600 hover:text-yellow-900 mr-3" data-id="${tx.transaction_id}">Edit</button>
                    <button class="delete-inventory text-red-600 hover:text-red-900" data-id="${tx.transaction_id}">Delete</button>
                  </td>
                </tr>
              `).join('') : 
              `<tr>
                <td colspan="7" class="px-6 py-4 text-center text-gray-500">No inventory transactions found</td>
              </tr>`}
          </tbody>
        </table>
      </div>`;
  ...
  function showInventoryTransactionsError(message) {
    maintenanceContent.innerHTML = `
      <div class="bg-white p-4 rounded shadow">
        <h3 class="text-lg font-semibold mb-2">Inventory Maintenance</h3>
        <p class="text-red-600">Error: ${message}</p>
        <button class="mt-2 bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600" 
  ...
  async function showInventoryModal(transaction = null) {
    // Populate ingredient dropdown
    const ingredientSelect = document.getElementById('inventoryIngredient');
    ...
    if (transaction) {
      inventoryModalTitle.textContent = 'Edit Inventory Transaction';
      document.getElementById('transactionId').value = transaction.transaction_id;
      document.getElementById('inventoryIngredient').value = transaction.ingredient_id;
      document.getElementById('inventoryTransactionType').value = transaction.transaction_type;
      document.getElementById('inventoryQuantity').value = transaction.quantity || '';
      document.getElementById('inventoryUnit').value = transaction.unit || '';
      document.getElementById('inventoryNotes').value = transaction.notes || '';
    } else {
      inventoryModalTitle.textContent = 'Add Inventory Transaction';
      inventoryForm.reset();
  ...
  async function handleInventorySubmit(e) {
    ...
    const formData = {
      ingredient_id: document.getElementById('inventoryIngredient').value,
      transaction_type: document.getElementById('inventoryTransactionType').value,
      quantity: parseFloat(document.getElementById('inventoryQuantity').value) || null,
      unit: document.getElementById('inventoryUnit').value,
      notes: document.getElementById('inventoryNotes').value || null
    };

    if (!formData.ingredient_id || !formData.transaction_type || !formData.quantity || !formData.unit) {
      showToast('Please fill in all required fields (Ingredient, Transaction Type, Quantity, Unit)', 'error');
      return;
    }
    ...
    try {
      const response = await fetch(isEdit ? `/api/inventory-maintenance/${transactionId}` : '/api/inventory-maintenance', {
        method: isEdit ? 'PUT' : 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formData)
      });
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || 'Failed to save inventory transaction');
      }

      showToast(`Inventory transaction ${isEdit ? 'updated' : 'added'} successfully`, 'success');
      inventoryModal.classList.add('hidden');
      await loadInventoryTransactions();
    } catch (err) {
      console.error('Error saving inventory transaction:', err);
      showToast(err.message || 'Failed to save inventory transaction', 'error');
    }
  }

  async function deleteInventoryTransaction(id) {
    if (!confirm('Are you sure you want to delete this inventory transaction?')) return;
    
    try {
      const response = await fetch(`/api/inventory-maintenance/${id}`, {
        method: 'DELETE'
      });

      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || 'Failed to delete inventory transaction');
      }

      showToast('Inventory transaction deleted successfully', 'success');
      await loadInventoryTransactions();
    } catch (err) {
      console.error('Error deleting inventory transaction:', err);
      showToast(err.message || 'Failed to delete inventory transaction', 'error');
    }
  }
